...
- reader: a simple Express server that reads that file and returns its content on GET `/`.

The two containers share an `emptyDir` volume mounted at `/usr/src/app/files`, which is how
the reader can see what the writer produces.  
